...
function DiscoverScreen({ onMatch, onOpenProfile }) {
  const deck = window.F2F_DISCOVER;
  const [idx, setIdx] = useStateD(0);
  const [drag, setDrag] = useStateD({ x: 0, y: 0, active: false });
  const [leaving, setLeaving] = useStateD(null); // {dir}
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: window.F2F_BG }}>
      <window.ScreenHeader title="Discover" />
        ...
        {!current && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column',
            ...
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#e7eae7',
              ...
              <window.Icon name="compass" size={30} stroke="#71717a" />
            </div>
    ...
    <div {...handlers} style={{
      ...
      <div style={{ padding: '22px 22px 0', flex: 1, overflow: 'hidden' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <window.Avatar initials={data.initials} size={70} />
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#71717a', fontSize: 14 }}>
          <window.Icon name="pin" size={15} stroke="#71717a" /> {data.distance}
        </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 9 }}>
          {data.interests.map(i => <window.Pill key={i} small on={data.shared?.includes(i)}>{i}</window.Pill>)}
        </div>
      </div>
      <div style={{ borderTop: '1px solid #f0f0f2', margin: '14px 0 0', padding: '12px 22px',
        ...
        <window.Icon name="info" size={17} stroke="#a1a1aa" />
