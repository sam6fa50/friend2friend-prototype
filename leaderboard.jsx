...
function LeaderboardScreen({ blocked }) {
  const me = window.F2F_ME, INK = window.F2F_INK, GREEN = window.F2F_GREEN;
  const board = window.F2F_LEADERBOARD;
  const [scope, setScope] = useStateL('Nearby');
  const [hidden, setHidden] = useStateL([]); // blocked names the user chose to hide

  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: window.F2F_BG }}>
      <window.ScreenHeader title="Leaderboard"
        right={<button style={infoBtn}><window.Icon name="info" size={20} stroke="#71717a" /></button>} />
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: `0 18px ${window.F2F_BOT_SAFE + 70}px` }}>
        {/* personal card */}
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                ...
                <span style={{ fontSize: 13, color: GREEN, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 2 }}>
                  <window.Icon name="arrowUp" size={14} stroke={GREEN} /> +120 this week
                </span>
              </div>
            </div>
          </div>
        ...
        {board.map(p => {
          ...
          return (
            <div key={p.rank} onClick={() => isBlocked && setHidden(h => [...h, p.name])} style={{
              ...
              <div style={{ position: 'relative' }}>
                <window.Avatar initials={p.initials} size={44} dim={isBlocked} />
                {!p.active && !isBlocked && (
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 2 }}>
                  {!isBlocked && <>
                    <window.Icon name="pin" size={11} stroke="#a1a1aa" />
              ...
              {!isBlocked && (
                <div style={{ textAlign: 'right' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 3, justifyContent: 'flex-end', fontSize: 11, color: '#a1a1aa' }}>
                    <window.Icon name="user" size={11} stroke="#a1a1aa" /> {p.connections}
...
function Medal({ bg, icon, iconColor }) {
  return (
    <div style={{ width: 30, height: 30, borderRadius: '50%', background: bg, display: 'flex',
      ...
      <window.Icon name={icon} size={17} fill={iconColor} stroke={iconColor} sw={1.4} />
    </div>
  );
}
function TrendArrow({ t }) {
  const GREEN = window.F2F_GREEN;
  if (t === 'up') return <window.Icon name="arrowUp" size={13} stroke={GREEN} />;
  if (t === 'down') return <window.Icon name="arrowDown" size={13} stroke="#ef4444" />;
  return <window.Icon name="minus" size={13} stroke="#a1a1aa" />;
}
