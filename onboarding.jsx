...
function Onboarding({ onDone, profile, setProfile }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: '#fff' }}>
      <div style={{ height: window.F2F_TOP_SAFE }} />
      {/* progress */}
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
        {step === 0 && (
          <div style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', textAlign: 'center' }}>
            <div style={{ width: 84, height: 84, borderRadius: 24, background: INK, margin: '0 auto 22px',
              ...
              <window.Icon name="zot" size={44} stroke="#fff" sw={2.4} />
            </div>
              ...
              {[['heart', 'Swipe to connect', 'Discover people who share your hobbies'],
                ...
                <div key={t} style={{ display: 'flex', alignItems: 'center', gap: 14, textAlign: 'left' }}>
                  <div style={{ width: 44, height: 44, borderRadius: 12, background: '#f0f4f0', flexShrink: 0,
                    display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <window.Icon name={ic} size={22} stroke={GREEN} />
                  </div>
        ...
        {step === 1 && (
          <div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
              {window.F2F_POPULAR.map(i => {
                const on = profile.interests.includes(i);
                return <window.Pill key={i} on={on} onClick={() => setProfile(p => ({ ...p,
                  interests: on ? p.interests.filter(x => x !== i) : [...p.interests, i] }))}>{i}</window.Pill>;
              })}
            </div>
        ...
        {step === 2 && (
          <div>
            ...
            <window.MapPlaceholder height={170} label={profile.region} radius />
      ...
      <div style={{ padding: `12px 24px ${window.F2F_BOT_SAFE + 8}px` }}>
