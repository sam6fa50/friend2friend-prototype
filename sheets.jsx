  ...
  return (
    <div onClick={onClose} style={{ position: 'absolute', inset: 0, zIndex: 80,
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        paddingBottom: window.F2F_BOT_SAFE, animation: 'f2fSheet .3s cubic-bezier(.2,.8,.3,1)' }}>
        <div style={{ width: 38, height: 5, borderRadius: 999, background: '#d4d4d8', margin: '10px auto 4px' }} />
        {title && <div style={{ padding: '6px 20px 10px', fontSize: 18, fontWeight: 700, color: window.F2F_INK,
...
function InterestSearchSheet({ selected, onToggle, onClose }) {
  const [q, setQ] = useStateS('');
  const [custom, setCustom] = useStateS('');
  const INK = window.F2F_INK, GREEN = window.F2F_GREEN;
  const pool = [...window.F2F_POPULAR, ...window.F2F_NICHE].filter((v, i, a) => a.indexOf(v) === i);
  ...
  return (
    <Sheet onClose={onClose} title="Find an interest" tall>
      <div style={{ padding: '12px 20px 8px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, background: window.F2F_BG,
          borderRadius: 12, padding: '10px 14px' }}>
          <window.Icon name="search" size={18} stroke="#a1a1aa" />
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '4px 20px 8px' }}>
        ...
        {norm && !exactExists && (
          <div onClick={() => addCustom(q)} style={{ display: 'flex', alignItems: 'center', gap: 10,
            ...
            <div style={{ width: 34, height: 34, borderRadius: 9, background: GREEN, display: 'flex',
              ...
              <window.Icon name="plus" size={20} stroke="#fff" />
            </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9, marginTop: 12 }}>
          {results.map(i => (
            <window.Pill key={i} on={selected.includes(i)} onClick={() => onToggle(i, !selected.includes(i))}>{i}</window.Pill>
          ))}
        </div>
...
// ── Block scope sheet ───────────────────────────────────────────────────────
function BlockSheet({ target, onConfirm, onClose }) {
  const [scopes, setScopes] = useStateS({ profile: true, geo: true, messages: true, leaderboard: false });
  const INK = window.F2F_INK;
    ...
    <Sheet onClose={onClose}>
      <div style={{ padding: '4px 22px 0', textAlign: 'center' }}>
        <div style={{ width: 56, height: 56, borderRadius: '50%', background: '#fee2e2', margin: '4px auto 12px',
          ...
          <window.Icon name="shield" size={26} stroke="#dc2626" />
        </div>
...
function ProfileDetailSheet({ user, onClose, onBlock, onMessage }) {
  const INK = window.F2F_INK, GREEN = window.F2F_GREEN;
  return (
    <Sheet onClose={onClose} tall>
      <div style={{ flex: 1, overflowY: 'auto', padding: '8px 22px 16px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          <window.Avatar initials={user.initials} size={64} />
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#71717a', fontSize: 13.5 }}>
              <window.Icon name="pin" size={14} stroke="#71717a" /> {user.distance}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 9 }}>
          {user.interests.map(i => <window.Pill key={i} on={user.shared?.includes(i)}>{i}</window.Pill>)}
        </div>
        <window.MapPlaceholder height={120} label={user.distance + ' · approx.'} />
      </div>
      <div style={{ borderTop: '1px solid #f0f0f2', padding: '12px 18px 0', display: 'flex', gap: 10 }}>
        <button onClick={() => onBlock({ name: user.name, initials: user.initials })} style={{ width: 52, borderRadius: 14,
          ...
          <window.Icon name="shield" size={20} stroke="#71717a" />
        </button>
        <button onClick={() => onMessage(user)} style={{ flex: 1, padding: '14px', borderRadius: 14, border: 'none',
          ...
          <window.Icon name="chat" size={18} stroke="#fff" /> Message
        </button>
      </div>
    </Sheet>
  );
...
function Toggle({ on }) {
  return (
    <div style={{ width: 46, height: 28, borderRadius: 999, background: on ? window.F2F_GREEN : '#d4d4d8',
...
Object.assign(window, { Sheet, InterestSearchSheet, BlockSheet, ProfileDetailSheet, Toggle });
