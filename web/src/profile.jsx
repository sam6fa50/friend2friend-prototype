...
import { Icon, Avatar, Pill, SocialIcon, MapPlaceholder,
...
export function ProfileScreen({ profile, setProfile, onOpenInterests, onPreview, blocked }) {
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', background: F2F_BG }}>
      <div style={{ flex: 1, overflowY: 'auto', paddingBottom: F2F_BOT_SAFE + 150 }}>
        {/* header */}
        <div style={{ padding: F2F_TOP_SAFE + 'px 20px 4px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            <Avatar initials={profile.initials} size={58} />
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 5, color: '#71717a', fontSize: 13 }}>
                <Icon name="pin" size={13} stroke="#71717a" /> {profile.region}
              </div>
            </div>
            <Icon name="edit" size={22} stroke="#71717a" />
          </div>
        </div>
        ...
        <div style={{ padding: '14px 20px 0', display: 'flex', flexDirection: 'column', gap: 22 }}>
          ...
          <Section label="Social media">
            <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
              ...
              <div style={{ width: 46, height: 46, borderRadius: 12, border: '1.5px dashed #cbd5d8',
                ...
                <Icon name="plus" size={22} stroke="#a1a1aa" />
              </div>
            </div>
          ...
          <Section label="Interests" right={<span style={{ fontSize: 12.5, color: '#a1a1aa' }}>{profile.interests.length}/20</span>}>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 9 }}>
              ...
              <div onClick={onOpenInterests} style={{ display: 'inline-flex', alignItems: 'center', gap: 6,
                ...
                <Icon name="plus" size={15} stroke="#52525b" /> See more
              </div>
            </div>
          </Section>
          ...
          <Section label="Location" right={
            ...
            <MapPlaceholder height={120} label={profile.region} radius={profile.shareLocation} />
          </Section>
                ...
                return (
                  <div key={b.id} onClick={() => {
                    ...
                    <div style={{ width: 38, height: 38, borderRadius: '50%', margin: '0 auto 6px',
                      ...
                      <Icon name={b.earned ? b.glyph : 'lock'} size={20}
  ...
  return (
    <div style={{ padding: '6px 0' }}>
      <input type="range" min={min} max={max} value={value} onChange={e => onChange(+e.target.value)}
        style={{ width: '100%', accentColor: F2F_INK, height: 6 }} />
    </div>
  );
}
